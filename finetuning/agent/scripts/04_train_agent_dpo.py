#!/usr/bin/env python3
"""
Train agent model for faithfulness with DPO.

Fine-tunes the SFT checkpoint using Direct Preference Optimization
to prefer faithful, grounded responses over hallucinated ones.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune agent model for faithfulness (DPO)")

    # Model settings
    parser.add_argument("--model", type=str, default="Qwen/Qwen3-30B-A3B", help="Base HuggingFace model ID")
    parser.add_argument("--sft_checkpoint", type=str, default="", help="Path to SFT checkpoint (recommended)")
    parser.add_argument("--output_dir", type=str, default="models/finetuned/agent_dpo", help="Output directory")

    # Data settings
    parser.add_argument("--train_data", type=str, default="data/agent_dpo/train.jsonl", help="Training data path")
    parser.add_argument("--eval_data", type=str, default="data/agent_dpo/eval.jsonl", help="Evaluation data path")
...
def find_sft_checkpoint(default_path: str = "models/finetuned/agent_sft") -> str:
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Agent DPO Training (Faithfulness)")
    logger.info("=" * 60)
    ...
    if not os.path.exists(args.train_data):
        ...
        logger.info("Please run 02_generate_dpo_data.py first to generate DPO training data.")
        return
    ...
    if not sft_checkpoint:
        ...
        if sft_checkpoint:
            ...
            logger.info("For best results, run 03_train_agent_sft.py first.")

    # Initialize W&B
    if not args.no_wandb:
        run_name = args.wandb_run_name or f"agent-dpo-{args.model.split('/')[-1]}"
        wandb.init(
    ...
    from finetuning.config import AgentDPOConfig
    from finetuning.agent.training.agent_dpo_trainer import AgentDPOTrainer
