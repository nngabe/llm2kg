#!/usr/bin/env python3
"""
Agent Ablation Study for agent_qa.py

Tests the impact of different agent features using the full 4-layer
agent evaluation framework with 14 metrics.
...
Usage:
    python benchmarks/agent_ablation_study.py
    python benchmarks/agent_ablation_study.py --configs baseline no_planning
    python benchmarks/agent_ablation_study.py --quick  # 4 test cases only
"""
...
def run_ablation_study(
    ...
    if test_cases_path is None:
        test_cases_path = "/app/benchmarks/agent_eval/golden/datasets/comprehensive_v1.json"

    logger.info("=" * 70)
    logger.info("AGENT ABLATION STUDY")
    logger.info("=" * 70)
    ...
    for config_name in config_names:
        ...
        try:
            ...
            # Run evaluation
            logger.info(f"  Running agent evaluation...")
...
def save_results(results: List[AblationResult], output_dir: str = "benchmarks/agent_eval/reports"):
...
def main():
    parser = argparse.ArgumentParser(description="Agent Ablation Study for agent_qa.py")
