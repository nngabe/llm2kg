...
Usage:
    ...
    python benchmarks/agent_eval.py --layers retrieval generation

    # CI/CD mode with failure on threshold
    python benchmarks/agent_eval.py --ci --fail-on-threshold

    # Generate golden dataset
    python benchmarks/agent_eval.py --generate-golden

    # Review generated test cases
    python benchmarks/agent_eval.py --review-golden

    # Run with custom config
    python benchmarks/agent_eval.py --config config.json
"""
...
from agent_eval.golden import GoldenDataset, GoldenDatasetGenerator
from agent_eval.golden.reviewer import run_review_cli
from agent_eval.reporting import JSONReporter, HumanReporter, CIReporter
...
def main():
    parser = argparse.ArgumentParser(
        description="Agent Evaluation Suite for GraphRAG + ReAct Agent"
    )
    ...
    parser.add_argument(
        "--test-cases",
        type=str,
        default="benchmarks/agent_eval/golden/datasets/default_v1.json",
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="benchmarks/agent_eval/reports",
    ...
    if args.generate_golden:
        ...
        output_path = Path(f"benchmarks/agent_eval/golden/datasets/{args.golden_name}.json")
