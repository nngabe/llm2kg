...
class SubgraphConnectivityMetric(BaseMetric):
    ...
    def _check_connectivity_neo4j(
        ...
        # NOTE: Neo4j does not allow parameters in variable-length path patterns,
        # so we use string formatting for max_path_length (safe since it's an int
        # from our own config, not user input)
        query = f"""
        MATCH (a:Entity {{name: $e1}}), (b:Entity {{name: $e2}})
        OPTIONAL MATCH path = shortestPath((a)-[*1..{self.max_path_length}]-(b))
